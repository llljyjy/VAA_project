...
drop Taiwan, and Euroarea, and rename the new dataframe
...
```{r}
library(readxl)
arg_inflation_data <- read_excel("C:/FirGhaz/VAA_project/data/ARG_inflation_Data.xlsx")
...
# View the first few rows to confirm Venezuela data is removed
head(cleaned_big_mac_data)
```

```{r}
# Save cleaned_big_mac_data dataframe to a CSV file
write.csv(cleaned_big_mac_data, file = "C:/FirGhAZ/VAA_project/data/aspatial/cleaned_big_mac_data.csv")
```
...
```{r}
BMIdata <- read_csv("C:/FirGhAZ/VAA_project/data/aspatial/cleaned_big_mac_data.csv")
```
```{r}
...
```{r}
write_rds(wmpb_World_BMI_2020, "C:/FirGhAZ/VAA_project/data/rds/wmpb_BMI_cleaned_2020.rds")
```
...
tm <- tm_shape(wmpb_World_BMI_2020) +
  tm_polygons("inflation", id = "name", popup.vars = c("Big Mac Index" = "usd_price", 
                                                       "Inflation" = "inflation", 
